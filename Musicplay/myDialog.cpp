...
void SetPaly(CDC* pDC, int x, int y, int btn, int dx,int paly)
{
	...
	if (btn == 1 && dx == 1)
	{
		pin.x = 448;
		pin.y = 612;
		myPolygon_1(&dcMem, pt, 4, cor1, &pin);//0x35, 0xC1, 255
		pin.x = 462;
		pin.y = 612;
		myPolygon_1(&dcMem, pt+4, 3, cor1, &pin);
	}
	else
	{
		myPolygon(&dcMem, pt, 4, cor0,0);//0x35, 0xC1, 255
		pin.x = 462;
		pin.y = 612;
		myPolygon(&dcMem, pt+4, 3, cor0, &pin);
	}
	////////////////////
	pin.x = 499;
	pin.y = 613;
	...
	if (paly==1)
	{
		...
		if (btn == 2 && dx == 1)
		{
			...
			myPolygon(&dcMem, pt, 6, cor1, &pin); //e5f5fb
		}
		else
		{
			myPolygon(&dcMem, pt, 6, cor0, &pin); //e5f5fb
		}
		g_playsus = MPALY;
	}
	else if (paly == 2)
	{
		...
		if (btn == 2 && dx == 1)
		{
			pin.x = 497;
			pin.y = 614;
			myPolygon(&dcMem, pt, 4, cor1, &pin);//0x35, 0xC1, 255
			pin.x = 507;
			pin.y = 615;
			myPolygon(&dcMem, pt + 4, 4, cor1, &pin);
		}
		else
		{
			pin.x = 497;
			pin.y = 614;
			myPolygon(&dcMem, pt, 4, cor0, &pin);//0x35, 0xC1, 255
			pin.x = 507;
			pin.y = 615;
			myPolygon(&dcMem, pt + 4, 4, cor0,&pin);
		}
		g_playsus = MSUSPEND;
	}
	...
	if (btn == 3 && dx == 1)
	{
		pin.x = 543;
		pin.y = 614;
		myPolygon(&dcMem, pt+4, 3, cor1, &pin);
		pin.x = 561;
		pin.y = 612;
		myPolygon(&dcMem, pt, 4, cor1, &pin);//0x35, 0xC1, 255
	}
