...
#define  STARTPARSTR   0   //这是待参数启动的程序
#define  SENDPARSTR    1   //这是已经启动远程发过来的参数需要释放内存
#define  LISTPARSTR    2   //这是播放列表右键传过来的参数
#define  BOTTONPARSTR   3   //这是按钮发过来的参数
#pragma pack(1)
typedef struct _MYLISTSTR
{
	CString MP3str;
	int dx =0;
	int memlen = 0;
	int lx=0;
}MYLISTSTR,*PMYLISTSTR,*LPMYLISTSTR;
#pragma pack()
class MicPalay
{
public:
...
public:
	//释放远程内存
	void VirFree(char* str,int len);
	void paly_1();
	//播放
	void paly(MYLISTSTR vear);
	//暂停
	void pause();
	//获取暂停状态
	int Getpause();
...
private:
	...
	BOOL        m_quit = FALSE;  //是否退出
	BOOL        m_stop = FALSE;
	int        m_paly = 0;
	BOOL       m_close = FALSE;  //退出进程状态
	int        m_addres = -1;     //外部传进来的内存长度
	int        m_memlen = 0;      //远程内存长度
