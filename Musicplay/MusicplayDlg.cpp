...
#define ID_HOTKEY  3
#define UM_TRAYNOTIFY  WM_USER+0x0010
#define VK_LEFTCILCK   512
...
BEGIN_MESSAGE_MAP(CMusicplayDlg, CDialogEx)
	...
	ON_WM_SYSCOMMAND()
	ON_COMMAND(ID_32786, &CMusicplayDlg::On32786)
END_MESSAGE_MAP()
...
BOOL CMusicplayDlg::OnInitDialog()
{
	...
	miscres1(str.GetBuffer(),str.GetLength());
...
int mp3strl = 0;
void CMusicplayDlg::miscres1(char* str ,int rl)
{
	if (strlen(str) <= 6)
	{
		return;
...
LRESULT CMusicplayDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case UM_TRAYNOTIFY:
	{
		...
		case VK_LEFTCILCK:
		{
			...
			CPoint point;
			GetCursorPos(&point);
		/*	if (no_menu_bar_width - point.x <= 245 && no_menu_bar_width - point.x > 0 &&
				no_menu_bar__height - point.y <= 67 && no_menu_bar__height - point.y > 0)
			{*/
				CString strToolTip = _T("红龙MP3播放器");
				CString str;
				int dx = MP3.GetMp3dx();
				str = m_list.GetItemText(dx, 0);
				if (dx >= 0)
				{
					strToolTip = str;
				}
		...
		/*		if (!DLGSH)
				{
					dlg.Setddd1(strToolTip);
					int w = strToolTip.GetLength() * 6 + 10;
					DLGSH = dlg.ShowWindow(1);
					dlg.SetWindowPos(&wndTopMost, 0, 0, 0, 0, SWP_NOREPOSITION | SWP_NOSIZE);
					dlg.MoveWindow(point.x - 30, point.y - 40, w, 20);
...
void CMusicplayDlg::On32786()//托盘菜单停止播放退出播放线程循环
{
	// TODO: 在此添加命令处理程序代码
	MP3.stop();
